...
def main():
    print("loading San Francisco network (walk access, no transit)...")
    r5 = R5Backend(OSM)
    print("network ready")
    today = dt.date.today()
    ...
    for seed in range(n_instances):
        riders = sample_origins("bayarea", 5, seed=seed, spread="clustered", clusters=1, cluster_sd_deg=0.012)
        modes = assign_modes(5, "walking", seed=seed)
        cands = candidates_for(riders, modes, 9, 10)
        print(f"seed {seed}: {len(cands)} candidates -> r5 walk matrix...", flush=True)
    ...
    df.to_csv("outputs/rideshare_sf.csv", index=False)
    cols = ["variance", "spread", "ede", "mean", "max", "jain", "opt_gap"]
    print("\nREAL San Francisco ride-share pickup (rider WALK access time, mean over instances):")
